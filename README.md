...
**Twitch chat bridge** (separate Railway service, same repo): set the service **Dockerfile** to **`Dockerfile.twitch-bridge`** (not the root `Dockerfile`). The root image runs a full Next.js build and needs **`DATABASE_URL` at build time**; the bridge doesn’t need that and builds fail if that variable isn’t passed to the second service. The slim bridge image installs only production npm deps (skips Prisma postinstall) and runs `node scripts/twitch-chat-bridge.mjs`. Leave **Start Command** empty unless you prefer `npm run twitch-bridge`. Do not use `./docker-entrypoint.sh` on the bridge. Set the bridge env vars (`TWITCH_BRIDGE_*`, `TWITCH_BOT_*`, `TWITCH_CHAT_INGEST_SECRET`, etc.). The **web** service needs the same `TWITCH_BOT_OAUTH` with **`user:write:chat`** (in addition to `chat:read`) so Synapse chat lines can be sent to Twitch via Helix. Access tokens expire in a few hours; add **`TWITCH_BOT_REFRESH_TOKEN`**, **`TWITCH_CLIENT_ID`**, and **`TWITCH_CLIENT_SECRET`** (same Twitch app) on **web and bridge** so tokens refresh automatically. Copy `refresh_token` from the OAuth token exchange response alongside `access_token`.
